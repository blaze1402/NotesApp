...
def notes_page():
    ...
    def addNote():
        if request.method == "POST":
            ...
            time = datetime.now(timezone("Asia/Kolkata")).strftime("%H:%M %d/%m/%Y")
...
def register_page():
    ...
    if form.validate_on_submit():
        ...
        flash(
            f"Account created successfully! You are now logged in as {create_user.username}!",
            category="success",
        )
    ...
    if form.errors != {}:
        for errors in form.errors.values():
            flash(
                f"There was an error with creating a user: {errors}", category="danger"
            )
...
def login_page():
    ...
    if form.validate_on_submit():
        ...
        if attempted_user and attempted_user.check_password_correction(
            ...
            flash(
                f"You have been successfully logged in as {attempted_user.username}!",
                category="success",
            )
            return redirect(url_for("notes_page"))
        else:
...
def edit(sno):
    if request.method == "POST":
        ...
        note.title = title
        note.desc = desc
        note.time = datetime.now(timezone("Asia/Kolkata")).strftime("%H:%M %d/%m/%Y")
        db.session.add(note)
        db.session.commit()
        return redirect(url_for("notes_page"))
